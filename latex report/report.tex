...
Diabetes is a chronic and widespread health condition that affects many individuals worldwide. Early prediction and risk assessment are essential for effective prevention and management. In this study, the application of Bayesian Networks (BNs) to model the probabilistic relationships between health indicators and diabetes risk is explored.  To enhance model performance, a comprehensive feature selection step values is employed, followed by the development of various Bayesian structures including Naïve Bayes, Hill Climbing, and domain knowledge-based techniques. The domain knowledge-based model achieved an AUC-ROC score of 83.57\%, outperforming pure data-driven approaches. The results demonstrate that Bayesian Networks offer an effective framework for diabetes risk prediction, providing healthcare professionals with an interpretable and reliable tool for patient assessment.
