...
\frenchspacing
\usepackage[left=0.6in, right=0.6in, top=0.65in, bottom=0.75in]{geometry}

%\hypersetup{pdfinfo={
%Title={A Probabilistic Approach to Diabetes Risk Assessment Using Bayesian Networks}
...
Diabetes is a chronic and widespread health condition that affects many individuals worldwide. Early prediction and risk assessment are essential for effective prevention and management. In this study, the application of Bayesian Networks (BNs) to model the probabilistic relationships between health indicators and diabetes risk is explored.  To enhance model performance, a comprehensive feature selection step is employed, followed by the development of various Bayesian structures including Naïve Bayes, Hill Climbing (with different scoring methods), Simulated Annealing, and domain knowledge-based techniques. The domain knowledge-based model achieved an AUC-ROC score of 83.57\%, outperforming all other experimented approaches. The results demonstrate that Bayesian Networks offer an effective framework for diabetes risk prediction, providing healthcare professionals with an interpretable and reliable tool for patient assessment.
...
Figure 1 presents the Domain Knowledge-Based Bayesian Network, illustrating the probabilistic dependencies between features.
 ...
 To establish the network structure, two main approaches are employed:
\begin{enumerate}
